...
export default function Login() {
  ...
  useEffect(() => {
    // Check karte hain ki kya user already logged in hai
    // Aapki code ke mutabik aap localStorage me 'user' save kar rahe hain
    const isUserInLocalStorage = localStorage.getItem("user");
    
    // Agar aap cookie check karna chahte hain (jaise aapne bataya tha), toh ye use karein:
    const isTokenInCookie = document.cookie.includes("token"); // 'token' ko apne real cookie name se badal dein agar kuch aur hai

    // Agar local storage ya cookie me data hai, toh dashboard par bhej do
    if (isUserInLocalStorage || isTokenInCookie) {
      navigate("/dashboard", { replace: true }); // replace: true se user back button dabakar wapas login par nahi aa payega
    }
  }, [navigate]);
