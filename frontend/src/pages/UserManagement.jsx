...
// 👇 TCodes ka Master Data (Grid ke liye)
...
export default function UserManagement() {
  ...
  // 👇 ADDED: tcodes array in editForm state
  const [editForm, setEditForm] = useState({
    name: "", email: "", username: "", userType: "", password: "", confirmPassword: "", tcodes: [] 
  });
  ...
  const handleEditChange = (e) => {
    ...
    // 👇 Agar admin select kiya, toh uske tcodes clear kar do
    if (name === "userType" && value === "admin") {
      setEditForm({ ...editForm, [name]: value, tcodes: [] });
      return;
    }
  ...
  const handleUpdateUser = async (e) => {
    ...
    const pwd = editForm.password;
    const confirmPwd = editForm.confirmPassword; 
    ...
    try {
      // 👇 Updated API Payload to include tcodes
      await api.put(`/users/${editingUser._id}`, {
        name: editForm.name,
        email: editForm.email,
        userType: editForm.userType,
        tcodes: editForm.tcodes // Send updated TCodes
      });
  ...
  return (
    <div style={styles.container}>
                ...
                users.map((u) => (
                  <tr key={u._id} style={styles.row}>
                    ...
                    <td style={{ ...styles.td, textAlign: "right" }}>
                      ...
                      {/* 👇 Edit Button Click: Load User Data including tcodes */}
                      <button 
                        onClick={() => { 
                          setEditingUser(u); 
                          setEditForm({ 
                            ...
                            password: "", 
                            confirmPassword: "",
                            tcodes: u.tcodes || [] // Set existing permissions
                          }); 
                        }} 
      ...
      {/* --- MODAL 1: HISTORY --- (Same as before) */}
      {historyModal && (
        <div style={styles.modalOverlay}>
          <div style={{ ...styles.modalContent, width: '750px' }}>
            {/* ... history modal content (unchanged) ... */}
            <h3 style={styles.modalTitle}>Activity Log: {historyModal.name}</h3>
            ...
            <div style={styles.modalFilterBar}>
              ...
              <div style={{ display: 'flex', gap: '5px' }}>
                <button onClick={handleFilterHistory} style={styles.applyBtn}>🔍 Filter</button>
                <button onClick={() => { setStartDate(""); setEndDate(""); openHistoryModal(historyModal, "", ""); }} style={styles.resetBtnSmall}>🔄 Reset</button>
              </div>
            </div>
            ...
            <div style={{ marginTop: '15px' }}>
              <button onClick={() => { setHistoryModal(null); setStartDate(""); setEndDate(""); }} style={{ ...styles.cancelBtn, width: '100%' }}>Close Window</button>
            </div>
          </div>
        </div>
      )}
      ...
      {freezeModal && (
        <div style={styles.modalOverlay}>
          <div style={{ ...styles.modalContent, width: '350px', textAlign: 'center' }}>
            <div style={{ fontSize: '40px', marginBottom: '10px' }}>{freezeModal.isFrozen ? "🔓" : "❄️"}</div>
            <h3>{freezeModal.isFrozen ? "Unfreeze Account?" : "Freeze Account?"}</h3>
            <p style={{ fontSize: '14px', color: '#555' }}>Are you sure you want to {freezeModal.isFrozen ? "unfreeze" : "freeze"} <b>{freezeModal.name}</b>?</p>
            <div style={{ display: 'flex', gap: '10px', marginTop: '20px' }}>
              <button onClick={confirmFreezeToggle} style={styles.saveBtn}>Yes, Confirm</button>
              <button onClick={() => setFreezeModal(null)} style={styles.cancelBtn}>Cancel</button>
            </div>
...
{editingUser && (
  <div style={styles.modalOverlay}>
    <div style={{ ...styles.modalContent, width: '600px', maxHeight: '85vh', overflowY: 'auto' }}>
      <h3 style={styles.modalTitle}>Edit User: {editingUser.name}</h3>
      <form onSubmit={handleUpdateUser} style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
        
        {/* 👇 FIX 1: Name aur Email ko ek sath line mein kiya aur un-comment kiya */}
        <div style={{ display: 'flex', gap: '10px' }}>
          <input name="name" value={editForm.name} onChange={handleEditChange} style={{...styles.input, flex: 1}} required placeholder="Full Name" />
          <input name="email" value={editForm.email} onChange={handleEditChange} style={{...styles.input, flex: 1}} required placeholder="Email Address" />
        </div>
        
        {/* 👇 FIX 2: Role select */}
        <div style={{ display: 'flex', gap: '10px' }}>
          <select name="userType" value={editForm.userType} onChange={handleEditChange} style={{...styles.input, flex: 1}}>
            <option value="user">User</option>
            <option value="admin">Admin</option>
          </select>
        </div>
              {/* 👇 PERMISSIONS GRID FOR EDIT */}
              {editForm.userType === "user" && (
                <div style={styles.permissionsContainer}>
                  <h4 style={styles.permissionsTitle}>Edit Module Permissions</h4>
                  <div style={styles.permissionsGrid}>
                    {availablePermissions.map((moduleData, index) => (
                      <div key={index} style={styles.moduleBox}>
                        <h5 style={styles.moduleName}>{moduleData.module}</h5>
                        <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
                          {moduleData.codes.map((codeObj) => (
                            <label key={codeObj.id} style={styles.checkboxLabel}>
                              <input 
                                type="checkbox" 
                                checked={editForm.tcodes?.includes(codeObj.id)}
              ...
              {/* Password Section */}
              <div style={{ background: '#f8fafc', padding: '15px', borderRadius: '8px', border: '1px dashed #cbd5e1', display: 'flex', flexDirection: 'column', gap: '10px' }}>
                <label style={{ fontSize: "12px", color: "#ef4444", fontWeight: "bold", margin: 0 }}>Reset Password (Optional)</label>
                <div style={{ display: 'flex', gap: '10px' }}>
                  <input type="text" name="password" placeholder="New Password" value={editForm.password} onChange={handleEditChange} style={{...styles.input, flex: 1}} />
                  <input type="text" name="confirmPassword" placeholder="Confirm Password" value={editForm.confirmPassword} onChange={handleEditChange} style={{...styles.input, flex: 1}} />
                </div>
              </div>
...
// ✨ STYLES
const styles = {
   backBtn: { background: 'none', border: 'none', color: '#64748b', cursor: 'pointer', fontWeight: '600', fontSize: '13px', left: '0', top: '0', padding: 0, marginBottom: '5px' },
  container: { minHeight: "100vh", backgroundColor: "#f8fafc", fontFamily: "'Inter', sans-serif" },
  mainContent: { padding: "20px", maxWidth: "1100px", margin: "0 auto" },
  pageHeader: { display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "20px" },
  pageTitle: { fontSize: "22px", fontWeight: "800", color: "#0f172a", margin: 0 },
  ...
  deleteBtn: { background: "#fee2e2", color: "#b91c1c", border: "none", padding: "5px 10px", borderRadius: "6px", cursor: "pointer", fontSize: "12px" },
  modalOverlay: { position: "fixed", top: 0, left: 0, right: 0, bottom: 0, background: "rgba(15, 23, 42, 0.6)", display: "flex", justifyContent: "center", alignItems: "center", zIndex: 1000 },
  modalContent: { background: "#fff", padding: "25px", borderRadius: "12px", boxShadow: "0 20px 25px -5px rgba(0,0,0,0.1)" },
  modalTitle: { margin: "0 0 20px 0", fontSize: "20px", color: "#1e293b" },
  logsList: { maxHeight: "300px", overflowY: "auto", margin: "15px 0", border: "1px solid #f1f5f9" },
  thMini: { textAlign: "left", padding: "8px", color: "#64748b" },
  tdMini: { padding: "8px" },
  input: { padding: "10px", borderRadius: "6px", border: "1px solid #cbd5e1", fontSize: "14px", boxSizing: "border-box", outline: "none", backgroundColor: "#fff" },
  saveBtn: { flex: 1, backgroundColor: "#2563eb", color: "#fff", border: "none", padding: "12px", borderRadius: "6px", cursor: "pointer", fontWeight: "600" },
  cancelBtn: { flex: 1, backgroundColor: "#f1f5f9", color: "#475569", border: "1px solid #cbd5e1", padding: "12px", borderRadius: "6px", cursor: "pointer", fontWeight: "600" },
  modalFilterBar: { display: 'flex', gap: '10px', marginBottom: '15px', background: '#f8fafc', padding: '10px', borderRadius: '8px' },
  modalDateInput: { padding: '5px', borderRadius: '4px', border: '1px solid #cbd5e1', fontSize: '12px' },
  applyBtn: { background: '#2563eb', color: '#fff', border: 'none', padding: '5px 12px', borderRadius: '4px', cursor: 'pointer', fontSize: '12px', fontWeight: '600' },
  resetBtnSmall: { background: '#fff', color: '#64748b', border: '1px solid #cbd5e1', padding: '5px 12px', borderRadius: '4px', cursor: 'pointer', fontSize: '12px' },
  ...
  // 👇 PERMISSIONS STYLES FOR EDIT MODAL
  permissionsContainer: { marginTop: "10px", padding: "15px", backgroundColor: "#f8fafc", borderRadius: "8px", border: "1px solid #e2e8f0" },
  permissionsTitle: { fontSize: "14px", fontWeight: "700", color: "#1e293b", margin: "0 0 10px 0" },
  permissionsGrid: { display: "grid", gridTemplateColumns: "1fr 1fr", gap: "15px" },
  moduleBox: { backgroundColor: "#ffffff", padding: "10px", borderRadius: "6px", border: "1px solid #e2e8f0" },
  moduleName: { margin: "0 0 8px 0", fontSize: "12px", color: "#3b82f6", fontWeight: "700", textTransform: "uppercase" },
  checkboxLabel: { display: "flex", alignItems: "center", cursor: "pointer", fontSize: "13px", color: "#475569" },
  checkbox: { marginRight: "8px", cursor: "pointer", accentColor: "#2563eb" },
